...
public function getAssignLimit() {
	...
	public function editProduct($product_id, $data) {
		...
		// 	print_r($product_id);
		// 	echo "here1";
		// 	$query = $this->db->query("INSERT INTO " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "', seller_id='".(int)$this->seller->getId()."', product_id = '" . (int)$product_id . "', date_added = NOW()");
		// }
		// else{
		// 	// print_r($product_id);
		// 	// echo "here11";
		// 	// die;
		// 	$query = $this->db->query("UPDATE " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "'  WHERE product_id = '" . (int)$product_id . "'");
		// }

		// IF EXISTS("select * from product_has_quantity where product_id = '" . (int)$product_id . "'")
		// 	$this->db->query("UPDATE " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "'  WHERE product_id = '" . (int)$product_id . "'");
		// ELSE
		// 	$this->db->query("INSERT INTO " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "', seller_id='".(int)$this->seller->getId()."', product_id = '" . (int)$product_id . "', date_added = NOW()");
			
		// $query = $this->db->query("INSERT INTO " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "', seller_id='".(int)$this->seller->getId()."', product_id = '" . (int)$product_id . "', date_added = NOW()");
// print_r($data);
// 		die;
		// $query .= "ON DUPLICATE KEY UPDATE " . DB_PREFIX . "product_has_quantity SET quantity_added_by_seller = '" . (int)$data['quantity'] . "', date_modified = NOW() ";
	...
	public function getProduct($product_id,$seller="") {
		$sql = "SELECT DISTINCT p.product_id,
			p.model,p.sku,p.upc,p.ean,p.jan,p.isbn,p.mpn,p.location,
